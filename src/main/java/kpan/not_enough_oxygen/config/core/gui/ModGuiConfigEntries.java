package kpan.not_enough_oxygen.config.core.gui;
...
import kpan.not_enough_oxygen.config.core.IConfigElement;
import kpan.not_enough_oxygen.config.core.ModConfigCategory;
import kpan.not_enough_oxygen.config.core.properties.AbstractConfigProperty;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyBlockPredicate;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyBool;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyDouble;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyEnum;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyFloat;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyInt;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyList;
import kpan.not_enough_oxygen.config.core.properties.ConfigPropertyLong;
...
public class ModGuiConfigEntries extends GuiListExtended {
    ...
    public static class EnumEntry<E extends Enum<E>> extends CycleValueEntry {

        public EnumEntry(ModGuiConfig owningScreen, ModGuiConfigEntries owningEntryList, ConfigPropertyEnum<E> property, E current, E defaultValue) {
        ...
        public boolean saveConfigElement() {
            if (enabled() && isChanged()) {
                ConfigPropertyEnum<E> prop = (ConfigPropertyEnum<E>) property;
                prop.setValue(((TypeEnum<E>) prop.getValueType()).getEnumClass().getEnumConstants()[currentIndex]);
                return property.requiresMcRestart();
            }
            return false;
        }
    ...
    public static class ListEntry extends ButtonEntry {
        protected final Object[] beforeValues;
        protected Object[] currentValues;

        public ListEntry(ModGuiConfig owningScreen, ModGuiConfigEntries owningEntryList, ConfigPropertyList configElement) {
            super(owningScreen, owningEntryList, configElement);
            beforeValues = configElement.getValues();
            currentValues = configElement.getValues();
            updateValueButtonText();
        }
        ...
        public void updateValueButtonText() {
            ...
            btnValue.displayString = sb.toString().replaceFirst(", ", "");
        }

        @Override
        public void valueButtonPressed(int slotIndex) {
            mc.displayGuiScreen(new ModGuiEditArray(owningScreen,
                    this,
                    getList(),
                    owningScreen.path + (owningScreen.path.isEmpty() ? "" : ".") + getList().getId(),
                    slotIndex,
                    currentValues,
                    enabled()));
        }
        ...
        @Override
        public boolean isDefault() {
            return Arrays.deepEquals(getList().getDefaultValues(), currentValues);
        }

        @Override
        public void setToDefault() {
            if (enabled()) {
                currentValues = getList().getDefaultValues();
                updateValueButtonText();
            }
        }
        ...
        public boolean saveConfigElement() {
            if (enabled() && isChanged()) {
                getList().setValues(currentValues);
                return configElement.requiresMcRestart();
            }
            return false;
        }

        private ConfigPropertyList getList() {
            return (ConfigPropertyList) configElement;
        }
    }
    ...
    public static class BlockPredicateEntry extends InputEntry {

        public BlockPredicateEntry(ModGuiConfig owningScreen, ModGuiConfigEntries owningEntryList, ConfigPropertyBlockPredicate property) {
            super(owningScreen, owningEntryList, property);
        }
    ...
    public static class CategoryEntry extends ListEntryBase {
        ...
        public CategoryEntry(ModGuiConfig owningScreen, ModGuiConfigEntries owningEntryList, ModConfigCategory category) {
            super(owningScreen, owningEntryList, category, getTranslatedName(category, owningScreen.path));
    ...
    public static abstract class PropertyEntry extends ListEntryBase {
        ...
        public PropertyEntry(ModGuiConfig owningScreen, ModGuiConfigEntries owningEntryList, AbstractConfigProperty property) {
            super(owningScreen, owningEntryList, property, I18n.format(property.getNameTranslationKey(owningScreen.path)));
        ...
        @Override
        public boolean enabled() {
            if (!canEditConfig(property.getSide()))
                return false;
            if (owningScreen.isWorldRunning)
                return !owningScreen.allRequireWorldRestart && !property.requiresWorldRestart();
            return true;
        }
    ...
    public static abstract class ListEntryBase implements IGuiConfigEntry {
        ...
        public ListEntryBase(ModGuiConfig owningScreen, ModGuiConfigEntries owningEntryList, IConfigElement configElement, String name) {
            ...
            drawLabel = !name.isEmpty();

            for (String line : name.split("\n")) {
                toolTip.add(ChatFormatting.GREEN + line);
            }

            String comment = I18n.format(configElement.getCommentTranslationKey(path)).replace("\\n", "\n");
            if (!comment.equals(configElement.getCommentTranslationKey(path))) {
            ...
            if (configElement instanceof AbstractConfigProperty) {
                for (String line : ((AbstractConfigProperty) configElement).getAdditionalComment().split("\n")) {
                    toolTip.add(ChatFormatting.YELLOW + line);
                }
            }

            if (!canEditConfig(configElement.getSide()))
                toolTip.add(TextFormatting.RED + I18n.format("kpan_config.message.cant_edit_server_config"));

            if (configElement.requiresWorldRestart() || owningScreen.allRequireMcRestart)
        ...
        public boolean requiresWorldRestart() {
            return configElement.requiresWorldRestart();
        }

        @Override
        public void onGuiClosed() { }

        public boolean canEditConfig(ConfigSide side) {
            if (side != ConfigSide.CLIENT) {
                // マルチプレイ時は無効
                if (owningScreen.isWorldRunning && ModMain.server == null)
                    return false;
            }
            return true;
        }
