...
import kpan.not_enough_oxygen.config.core.gui.ModGuiConfigEntries.ListEntry;
 ...
 * @author bspkrs
 */
public class ModGuiEditArray extends GuiScreen {
    private final ModGuiConfig parentScreen;
    public final ListEntry parentEntry;
    protected ConfigPropertyList propertyList;
    protected ModGuiEditArrayEntries entryList;
    protected GuiButtonExt btnUndoChanges, btnDefault, btnDone;
    protected final String path;
    protected final String title;
    protected final String titleLine2;
    protected final @Nullable String titleLine3;
    ...
    public ModGuiEditArray(ModGuiConfig parentScreen, ListEntry parentEntry, ConfigPropertyList propertyList, String path, int slotIndex, Object[] currentValues, boolean enabled) {
        ...
        title = parentScreen.title;
        if (path.isEmpty()) {
            titleLine2 = I18n.format(propertyList.getNameTranslationKey(path));
            titleLine3 = null;
        } else {
            titleLine2 = path.replace(".", " > ");
            titleLine3 = I18n.format(propertyList.getNameTranslationKey(path));
        }

        String comment = I18n.format(propertyList.getCommentTranslationKey(path)).replace("\\n", "\n");
        if (!comment.equals(propertyList.getCommentTranslationKey(path))) {
        ...
        for (String line : ((AbstractConfigProperty) propertyList).getAdditionalComment().split("\n")) {
            toolTip.add(ChatFormatting.YELLOW + line);
        }

        if (!canEditConfig(propertyList.getSide()))
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 2000) {
            ...
            // reset
            this.currentValues = propertyList.getDefaultValues();
            this.entryList = createEditArrayEntries();
    ...
    protected ModGuiEditArrayEntries createEditArrayEntries() {
        return new ModGuiEditArrayEntries(this, this.mc, this.propertyList, this.beforeValues, this.currentValues);
    }
    ...
    private boolean canEditConfig(ConfigSide side) {
        if (side != ConfigSide.CLIENT) {
            // マルチプレイ時は無効
            if (parentScreen.isWorldRunning && ModMain.server == null)
                return false;
        }
        return true;
    }
